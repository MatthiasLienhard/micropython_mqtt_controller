...
class MicroGUI(display.TFT): 
    ...
    def add_rotary_encoder(self, clk, dt):
        self.rotary_encoder=DEC(0,clk, dt)
        self.rotary_encoder.pause()
    ...
    def init(self, *args, **kwargs) :
        ...
        if hasattr(self, 'rotary_encoder'):
            loop.create_task(self.handle_rotary_encoder())
    ...
    async def handle_rotary_encoder(self, freq=4):
        self.rotary_encoder.resume()
        self.rotary_encoder.clear()
        while self.initiated:
            val=self.rotary_encoder.count_and_clear()
            if val!=0:
                print(val)
...
class Button(Label):
    def __init__(self, text, command,margin=10,decoration='{}',halign=1,valign=1):
        #todo: margins, minsize, maxsize
        super().__init__(text,decoration, halign,valign)
